...
def load_piecewise_sinusoidal(t0=96, n=1000, return_numpy=False, return_torch=False, random_state=0):
    ...
    if ps.return_torch:
        return TensorDataset(torch.from_numpy(ps.x), torch.from_numpy(ps.fx), torch.from_numpy(ps.masks))
        #return torch.from_numpy(ps.x), torch.from_numpy(ps.fx), torch.from_numpy(ps.masks)
...
class PiecewieseSinusoidal():
    ...
    def __init__(self, t0=96, n=1000, return_numpy=False, return_torch=False, random_state=0):
        ...
        self.fx = self.fx + self.rng.normal(size=self.fx.shape)
